...
func (r *InvoiceRepository) Save(invoice *domain.Invoice) error {
	...
	(id, account_id, status, description, 
	amount, payment_type, card_last_digits, 
	created_at, updated_at) 
	VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
	`, invoice.ID, invoice.AccountID, invoice.Status, invoice.Description,
		invoice.Amount, invoice.PaymentType,
...
func (r *InvoiceRepository) FindById(id string) (*domain.Invoice, error) {
	...
	err := r.db.QueryRow(`
	SELECT id, account_id, status, description, amount, payment_type, card_last_digits, 
...
func (r *InvoiceRepository) FindByAccountId(accountId string) ([]*domain.Invoice, error) {
	rows, err := r.db.Query(`
	SELECT id, account_id, status, description, amount, payment_type, card_last_digits, created_at, updated_at FROM invoices WHERE account_id = $1
	`, accountId)
	if err != nil {
		return nil, err
	}
